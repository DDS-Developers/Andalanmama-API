...
<div id="update-book">
    <form action="{{ route('book.update', ['book' => $book ]) }}" method="POST" class="col-6 mx-auto">
        {{ csrf_field() }}
        @method('PUT')

        <div class="form-group">
            <label for="name">Nama buku resep</label>
            <input type="text" class="form-control" name="title" placeholder="Nama buku resep" v-model="book.title">
        </div>
        ...
        <div class="form-group custom-control custom-switch">
            ...
            <input type="checkbox" value="1" class="custom-control-input" v-model="book.status" name="status" id="status" true-value="1" false-value="0" checked>
            <label class="custom-control-label" for="status">Published The Recipe Book?</label>
        </div>
        <div class="form-group">
            <button class="btn btn-primary">Simpan perubahan</button>
            <a href="{{ route('admin.recipebooklist') }}" class="btn btn-link">Kembali</a>
        </div>
    </form>
        ...
        var app = new Vue({
            ...
            data () {
                return {
                    ...
                    recipesForm: {!! json_encode($book->recipes) !!}
                }
            },
            mounted () {
                this.fetchApi();
